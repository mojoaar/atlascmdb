...
8. **Knex `.clone().count()`**: Apply BEFORE `.orderBy()` and `.limit()` to get total rows.
 ...
 25. **`GROUP_CONCAT` is SQLite-only**: When concatenating strings within a query group (e.g., aggregating user roles), branch the syntax based on the database client configuration. Use `string_agg(column, ',')` for PostgreSQL and `GROUP_CONCAT(column)` for SQLite.
