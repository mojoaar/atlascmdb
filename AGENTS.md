...
8. **Knex `.clone().count()`**: Apply BEFORE `.orderBy()` and `.limit()` to get total rows.
 ...
 23. **Paginated count query on PostgreSQL**: To prevent Postgres syntax errors when executing paginated list counts on queries containing `select()`, always call `.clearSelect()` on the cloned query before calling `.count()`. Also, coerce the returned total using `Number(countResult.total)` because PostgreSQL driver returns count as a string.
 24. **Type Alignment for Joins on PostgreSQL**: Primary keys use `t.uuid('id')` throughout the database. All referencing audit/foreign key columns (including `createdBy` and `updatedBy`) must also be typed as `t.uuid()`. Avoid using generic text fields (`t.text()`) for columns that store or are joined against UUIDs, as PostgreSQL will reject comparison joins with a `text = uuid` operator type mismatch.
 25. **`GROUP_CONCAT` and `GROUP BY` differences on PostgreSQL**: 
     - **String Aggregation**: `GROUP_CONCAT` is SQLite-only. When concatenating strings within a query group (e.g., aggregating user roles), branch the syntax: use `string_agg(column, ',')` for PostgreSQL and `GROUP_CONCAT(column)` for SQLite.
     - **Group By functional-dependency**: When using aggregated columns with a `GROUP BY`, PostgreSQL requires all selected non-aggregated columns to be either in the `GROUP BY` or functionally dependent on a grouped primary key of their own table. Columns selected from separate joined relations (e.g. `manager.displayName`) must have their respective relation's PK (`manager.id`) added to the `GROUP BY` list.
      - **camelCase in raw SQL / orderByRaw**: PostgreSQL folds unquoted identifiers to lowercase, meaning `LOWER(users.displayName)` will look for `users.displayname` and fail. Always use Knex `??` identifier placeholders (e.g. `orderByRaw('LOWER(??) asc', ['users.displayName'])`) to ensure columns are properly quoted during compilation.
 26. **PostgreSQL Schema and Runtime Gotchas (Search, Booleans, Counts)**:
     - **Case-Insensitive Search (LIKE vs ILIKE)**: PostgreSQL `LIKE` is strictly case-sensitive, while SQLite `LIKE` is case-insensitive for ASCII. For database-agnostic case-insensitive searches, use `likeOperator(db)` from `@/lib/db` (which returns `'ilike'` on Postgres, and `'like'` on SQLite).
     - **Boolean Columns & Writes**: PostgreSQL uses a strict `BOOLEAN` column type. Never insert/update with integer indicators `1` and `0` on Postgres; always write strict JavaScript booleans `true` and `false`. Also, ensure API outputs normalize boolean attributes (e.g. `!!pref.enabled`) so the frontend receives a uniform type regardless of the database.
     - **Count Coercion**: PostgreSQL returns table `.count()` outputs as strings, whereas SQLite returns them as numbers. Always coerce counts to `Number(countResult.total)` to prevent layout rendering issues and type mismatches.
 27. **Third-Party Browser Extension Errors**: Browser extensions (Firefox's `moz-extension://` and Chrome's `chrome-extension://`) can throw synchronous errors or unhandled promise rejections that propagate to window event handlers and trigger Next.js/Turbopack's development-mode error overlay. To prevent development workflow disruption, `src/app/layout.js` registers event listeners for `'error'` and `'unhandledrejection'` at the capture phase to intercept and stop propagation of these external extension errors, while fully preserving native app errors.
